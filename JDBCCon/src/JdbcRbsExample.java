...
class JdbcRbsExample extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		ResultSet rs = null;	String filename = null;
		String cmd = null;
				
if (target.equals(exportButton)){  		
					cmd="select * from alarms";	
					filename = "EveryAlarm.csv";
	}		

else if(target.equals(alarmsperRBSButton)) {
	cmd="select RBS_ID,count(*) AS 'alarmsperRBS' from alarms group by RBS_ID";	
	filename = "AlarmsperRBS.csv";
}

else if(target.equals(allAlarmsforRBSButton)) {
	
	String rbsid=rBSIDTF.getText();
	cmd="select *  from alarms where RBS_ID = " + rbsid;	
	filename = "AlarmsforRBS.csv";
}

else if(target.equals(alarmsAfterButton)) {
	
	String date=timeTF.getText();
	cmd="select *  from alarms where Date > '" + date +"'";	
	filename = "AlarmsAfter.csv";
}
